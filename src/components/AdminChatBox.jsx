...
import {createMessagesApi, getMessagesApi, getUser} from "../api/api";
import {UserContext} from "../utils/user-context";

const AdminChatBox = ({chat, currentUser}) => {
    ...
    return (
        <>
            {chat ? (
                    ...
                    <div className="flex-1 p-4 overflow-y-auto h-[40rem]">
                        {messages.map((message, index) => (
                            <div key={index}
                                ...
                                <div
                                    className={`max-w-xs px-4 py-2 my-1 rounded-lg ${message.senderId === currentUser ? 'bg-blue-500 text-white' : 'bg-gray-200 text-black'}`}>
                                    <p>{message.text}</p>
                ...
                <div>
                    Click on a person to start conversation
                </div>
            )}
        </>
    );
